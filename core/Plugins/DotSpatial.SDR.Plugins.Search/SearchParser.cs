...
namespace DotSpatial.SDR.Plugins.Search
{
    ...
    public class StreetAddressParser
    {
        ...
        public static StreetAddress Parse(string query, bool usePreTypes)
        {
            ...
            if (input.Length <= 0) return new StreetAddress();
            ...
            var last = input.Length - 1;  // the last position available for parsing
            var result = new StreetAddress();  // final return result
            ...
            else
            {
                if (last - 1 > 0)
                {
                    if (RangedSubunits.ContainsValue(input[last - 1]) || RangedSubunits.ContainsKey(input[last - 1]))
                    {
                    ...
                    if (UnrangedSubunits.ContainsValue(input[1]) || UnrangedSubunits.ContainsKey(input[1]))
                    {
                        ...
                        if (input.Length > 2)
                        {
                            ...
                            if (Directionals.ContainsValue(input[2]) || Directionals.ContainsKey(input[2]))
                            {
                                ...
                                predirPos = "2";
                                if (input.Length > 3)  // a final check for array length criteria
                                {
            ...
            if (strNamePos == null) return result;  // no street name avail, return it as is

            // assemble the temp streetname value
            ...
            // check for use of PreTypes or not
            if (!usePreTypes)
            {
                // no pretypes considered set to street name and return home
                result.StreetName = strNameTmp.Trim();
                return result;
            }

            // using pretypes so break this apart once more to search for pretype values
            var strNameArr = strNameTmp.ToUpper().Split(' ');
            ...
            if (strNameArr.Length >= 2)
            {
                switch (strNameArr[0])
                {
                    ...
                    case "US":
                        ...
                        break;
                    default:  // no pretype avail set the whole thing as streetname
                        result.StreetName = strNameTmp.Trim();
                        break;
                }
            }
